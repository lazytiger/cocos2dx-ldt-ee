--------------------------------
-- @module CCControlSwitch

--------------------------------
-- @function [parent=#CCControlSwitch] initWithMaskSprite
-- @param self
-- @param CCSprite#CCSprite maskSprite
-- @param CCSprite#CCSprite onSprite
-- @param CCSprite#CCSprite offSprite
-- @param CCSprite#CCSprite thumbSprite
-- @return #bool

--------------------------------
-- @function [parent=#CCControlSwitch] create
-- @param self
-- @param CCSprite#CCSprite maskSprite
-- @param CCSprite#CCSprite onSprite
-- @param CCSprite#CCSprite offSprite
-- @param CCSprite#CCSprite thumbSprite
-- @return #CCControlSwitch

--------------------------------
-- @function [parent=#CCControlSwitch] initWithMaskSprite
-- @param self
-- @param CCSprite#CCSprite maskSprite
-- @param CCSprite#CCSprite onSprite
-- @param CCSprite#CCSprite offSprite
-- @param CCSprite#CCSprite thumbSprite
-- @param CCLabelTTF#CCLabelTTF onLabel
-- @param CCLabelTTF#CCLabelTTF offLabel
-- @return #bool

--------------------------------
-- @function [parent=#CCControlSwitch] create
-- @param self
-- @param CCSprite#CCSprite maskSprite
-- @param CCSprite#CCSprite onSprite
-- @param CCSprite#CCSprite offSprite
-- @param CCSprite#CCSprite thumbSprite
-- @param CCLabelTTF#CCLabelTTF onLabel
-- @param CCLabelTTF#CCLabelTTF offLabel
-- @return #CCControlSwitch

--------------------------------
-- @function [parent=#CCControlSwitch] setOn
-- @param self
-- @param #bool isOn
-- @param #bool animated

--------------------------------
-- @function [parent=#CCControlSwitch] setOn
-- @param self
-- @param #bool isOn

--------------------------------
-- @function [parent=#CCControlSwitch] isOn
-- @param self
-- @return #bool

--------------------------------
-- @function [parent=#CCControlSwitch] hasMoved
-- @param self
-- @return #bool

--------------------------------
-- @function [parent=#CCControlSwitch] setEnabled
-- @param self
-- @param #bool enabled

--------------------------------
-- @function [parent=#CCControlSwitch] locationFromTouch
-- @param self
-- @param CCTouch#CCTouch touch
-- @return CCPoint#CCPoint

--------------------------------
-- @function [parent=#CCControlSwitch] ccTouchBegan
-- @param self
-- @param CCTouch#CCTouch pTouch
-- @param CCEvent#CCEvent pEvent
-- @return #bool

--------------------------------
-- @function [parent=#CCControlSwitch] ccTouchMoved
-- @param self
-- @param CCTouch#CCTouch pTouch
-- @param CCEvent#CCEvent pEvent

--------------------------------
-- @function [parent=#CCControlSwitch] ccTouchEnded
-- @param self
-- @param CCTouch#CCTouch pTouch
-- @param CCEvent#CCEvent pEvent

--------------------------------
-- @function [parent=#CCControlSwitch] ccTouchCancelled
-- @param self
-- @param CCTouch#CCTouch pTouch
-- @param CCEvent#CCEvent pEvent

return nil
